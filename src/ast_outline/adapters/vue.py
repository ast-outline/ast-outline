...
from .html import (
    ...
    node_to_decls as _html_node_to_decls,
    recover_elements as _html_recover_elements,
)
from .typescript import walk_module as _ts_walk_module
from .typescript import collect_imports as _ts_collect_imports
from ..core import (
    Declaration,
    ParseResult,
)
    ...
    def parse(self, path: Path) -> ParseResult:
        ...
        for child in tree.root_node.named_children:
            ...
            if tag == "template":
                ...
                total_errors += _parse_style(child, src, decls, imports)
...
def _parse_style(
    ...
    section_decls: list[Declaration] = []
    # CSS ``@import`` text is source-true and position-independent, so it
    # merges straight into the file-level imports list (the base CSS
    # adapter surfaces these too; CSS imports carry no import_regions).
    _css_walk_top_level(tree.root_node, section_bytes, section_decls, imports)
    byte_offset = raw.start_byte
    line_offset = src[: raw.start_byte].count(b"\n")
    for d in section_decls:
        _adjust_decl(d, byte_offset, line_offset)
